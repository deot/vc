...
const getFestival = (oDate, lunarInfo) => {
	...
	return arrHolidays.filter((item) => item); };
...
const init = (oDate) => {
	...
	let q;
	let solarTerm = "";
	let holiday = ''; if (_date != s.getDate()) {
	...
	return {
		...
		// 阴历日期数字对应汉字日期
		lDate: lunarInfo.lunarDay % 10 == 0 ? ["初十", "二十", "三十"][lunarInfo.lunarDay / 10 - 1] : k[parseInt(lunarInfo.lunarDay / 10, 10)] + g[parseInt(lunarInfo.lunarDay % 10, 10)], solarTerm, // 节气
...
const date2holiday = (dateStr) => {
	let oDate = new Date(dateStr.replace(/\-/g, '/')); return init(oDate);
};
