<template>
	<div class="date-picker-portal-demo">
		<p>
			portal 控制下拉的挂载位置：true（默认）挂到 body，滚动容器滚动时由 JS 跟随，触发器滚出容器可视区时隐藏；
			false 挂到组件根节点内，随容器一起滚动，超出容器的部分被裁剪。打开后滚动灰色容器对比。
		</p>
		<div class="date-picker-portal-demo__row">
			<div
				v-for="portal in [true, false]"
				:key="String(portal)"
				:data-portal="String(portal)"
				class="date-picker-portal-demo__col"
			>
				<p>portal = {{ portal }}</p>
				<div class="date-picker-portal-demo__scroll">
					<div class="date-picker-portal-demo__inner">
						<DatePicker
							v-model="values[String(portal)]"
							:portal="portal"
							:portal-class="`date-picker-portal-demo__popup-${portal}`"
...
const handleReady = portal => requestAnimationFrame(() => {
	const parent = document.querySelector(`.date-picker-portal-demo__popup-${portal}`)?.parentElement;
	mounts[String(portal)] = parent === document.body ? 'body' : '组件根节点内';
});
</script>

<style lang="scss">
.date-picker-portal-demo {
	padding: 40px;
