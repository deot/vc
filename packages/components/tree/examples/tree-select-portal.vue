<template>
	<div class="tree-select-portal-demo">
		<p>
			portal 控制下拉的挂载位置：true（默认）挂到 body，滚动容器滚动时由 JS 跟随，触发器滚出容器可视区时隐藏；
			false 挂到组件根节点内，随容器一起滚动，超出容器的部分被裁剪。打开后滚动灰色容器对比。
		</p>
		<div class="tree-select-portal-demo__row">
			<div
				v-for="portal in [true, false]"
				:key="String(portal)"
				:data-portal="String(portal)"
				class="tree-select-portal-demo__col"
			>
				<p>portal = {{ portal }}</p>
				<div class="tree-select-portal-demo__scroll">
					<div class="tree-select-portal-demo__inner">
						<TreeSelect
							v-model="values[String(portal)]"
							:data="data"
							:max="99"
							:portal="portal"
							:portal-class="`tree-select-portal-demo__popup-${portal}`"
...
const handleReady = portal => requestAnimationFrame(() => {
	const parent = document.querySelector(`.tree-select-portal-demo__popup-${portal}`)?.parentElement;
	mounts[String(portal)] = parent === document.body ? 'body' : '组件根节点内';
});
</script>

<style lang="scss">
.tree-select-portal-demo {
	padding: 40px;
